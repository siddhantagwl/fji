...
class KPIDataProcessor:
    ...
    def validate_columns(self, df: pd.DataFrame):
        ...
        self.python_errors_list = [
            entry for entry in self.python_errors_list
            if not 'Columns mismatch. Extra: "[]"' in entry
        ]
    ...
    def prepare_staff_data(self, df_staff_exp_kpi: pd.DataFrame) -> Tuple[List[str], dict]:
        ...
        active_staff = df_staff_exp_kpi.loc[
            df_staff_exp_kpi['Active'] == 'Y', 'Names'
        ].values.tolist()
    ...
    def generate_summary_reports(self, df: pd.DataFrame, include_overall: str,
        ...
        from summary_photographers import summary_of_photographers_all_projects, summary_of_photographers_project_wise
        from summary_photostackers import summary_of_photostackers_all_projects, summary_of_photostackers_project_wise
        from summary_retouchers import summary_of_retouchers_all_projects, summary_of_retouchers_project_wise
        ...
        df_photographers = summary_of_photographers_all_projects(
            df, include_overall, date_ranges['start_date'], date_ranges['end_date']
        )
        df_photographers_project_wise = summary_of_photographers_project_wise(
            df, date_ranges['start_date'], date_ranges['end_date']
        )
        ...
        df_photography_summary_project_wise = summary_of_photography_project_wise(df)
    ...
    def filter_for_active_staff(self, summary_data: dict, active_staff: List[str]) -> dict:
        ...
        summary_data['photographers'] = utils.filter_index_for_active_staff(
            summary_data['photographers'], active_staff
        )
        summary_data['photostackers'] = utils.filter_index_for_active_staff(
            summary_data['photostackers'], active_staff
        )
        summary_data['retouchers'] = utils.filter_index_for_active_staff(
            summary_data['retouchers'], active_staff
        )
    ...
    def calculate_yearly_performance(self, df: pd.DataFrame, df_staff_exp_kpi: pd.DataFrame,
                                   date_ranges: dict, staff_and_their_categories: dict,
                                   active_staff: List[str], ppj_dict: dict, sub_catg_list: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    def create_excel_output(self, df: pd.DataFrame, summary_data: dict, df_output_sheets: pd.DataFrame,
        ...
        datasheet_aggregated_filepath = os.path.join(
            self.curr_path, config.PYTHON_CODES_FOLDER_NAME,
            config.INTERMEDIATE_FOLDER_NAME, config.AGGREGATED_DATASHEET_OUTPUT_FILENAME
        )
        ...
        excel_filepath = os.path.join(
            self.curr_path, config.PYTHON_CODES_FOLDER_NAME,
            config.INTERMEDIATE_FOLDER_NAME, config.INTERMEDIATE_OUTPUT_FILENAME
        )
    ...
    def _write_main_tables(self, writer, df_list: List[pd.DataFrame], summary_sheet: str) -> pd.DataFrame:
        ...
        for i, temp_df in enumerate(df_list):
            df_table_rows_map.loc[len(df_table_rows_map)] = [
                temp_df.index.name, r + 1, r + 1 + len(temp_df)
            ]
            r += len(temp_df) + 2

        utils.multiple_dfs_on_same_sheet(
            writer, df_list, summary_sheet, spaces=1, row=config.START_ROW_EXCEL_OUTPUT
        )

        return df_table_rows_map


    def _write_detailed_sheets(self, writer, workbook, summary_data: dict,
                              df_output_sheets: pd.DataFrame, date_ranges: dict, df: pd.DataFrame):
    ...
    def _write_yearly_performance_sheet(self, writer, workbook, df_output_sheets: pd.DataFrame,
                                       df_yearly_performance: pd.DataFrame, df_points_chart: pd.DataFrame,
                                       date_ranges: dict):
        ...
        write_yearly_performance_sheet(
            writer, workbook, sheetname, df_yearly_performance, df_points_chart,
            date_ranges['yp_start_date'], date_ranges['yp_end_date']
        )
    ...
    def process_kpi_data(self):
        ...
        try:
            ...
            df_yearly_performance, df_points_chart = self.calculate_yearly_performance(
                df, df_staff_exp_kpi, date_ranges, staff_and_their_categories,
